    ...
    */
    ...
    const handleExecuteRecommendation = () => {
        ...
        if (projects.length === 0) {
            toast.error(t('dashboard.noActiveProjects', "No active projects found. Please create a project first."));
            return;
        }

        setConfirmModal({
            isOpen: true,
            title: t('dashboard.createTaskTitle', 'Create Task from Oracle'),
            message: t('dashboard.createTaskMessage', 'Create a high-priority task based on this recommendation?'),
            action: async () => {
                try {
                    ...
                    toast.success(t('dashboard.taskCreatedSuccess', 'Task created successfully!'));
                } catch (e) {
                    console.error(e);
                    toast.error(t('dashboard.taskCreatedError', 'Failed to create task'));
                }
            }
        });
    };
        ...
        <div className={`min-h-screen bg-slate-50/50 p-6 space-y-8 animate-in fade-in duration-700 ${zoomedTile ? 'overflow-hidden' : ''}`}>
            ...
            <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-6 relative z-10">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-brand-200 font-medium mt-1">Real-Time Agency Intelligence • {new Date().toLocaleDateString()}</p>
                </div>
